...
type Props = {
  ...
  title: string;
  todayLabel?: string; // ej. "Hoy: martes / viernes"
};
  ...
  return (
    <div className="relative rounded-3xl border border-[rgba(15,23,42,0.08)] bg-white/85 backdrop-blur-md shadow-[0_10px_30px_rgba(0,0,0,0.06)] px-4 py-4 md:px-6 md:py-5">
      <div className="flex items-center gap-4">
        {/* Thumbnail con borde y brillo */}
        <div className="relative shrink-0">
          <img
            src={cover}
            alt={title}
            className="w-20 h-20 md:w-24 md:h-24 rounded-2xl object-cover shadow-md ring-1 ring-black/5"
          />
        ...
        <div className="flex-1">
          <h3 className="text-2xl md:text-3xl font-semibold tracking-wide text-slate-800">
            {title}
          </h3>
          {todayLabel && (
